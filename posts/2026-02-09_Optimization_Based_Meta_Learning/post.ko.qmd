...
마지막 내용인 Case Study에서는 CVPR 2020에서 진행되었던 [EarthVision Workshop](https://www.grss-ieee.org/wp-content/uploads/earthvision2020/)에서 Meta Learning을 활용해 Few-shot Land Cover Classification을 한 것에 대해 소개했다. (@Russwurm_2020_CVPR_Workshops)

![Land Cover Clasification](./images/33p_land_cover_classification.png)

해당 논문에서 풀고자 했던 문제는 공개되어 있는 위성에서 찍은 땅 이미지가 도심지역(Urban)인지, 농장지대(Agriculture)인지 등을 구분해내는 문제였고, 이때 공개되어 있는 데이터셋인 SEN12MS (@schmitt2019sen12ms) 와 DeepGlobe (@demir2018deepglobe) 를 사용했다. 그런데 실제 해당 데이터셋을 사용하고 활용할 때, 현실적인 문제를 겪는데, 인공위성 이미지를 직접적으로 활용하다 보니 이미지의 각 픽셀이 어떤 땅인지 표시하는 작업 자체가 비용과 시간이 많이 든다. 또한 전 세계의 지역마다 땅의 생김새가 다른 문제, 통계학적으로 보면 Distribution Shift가 발생하게 된다.

![Framing as a meta-learning problem](./images/34p_Framing_meta_Learning.png)
...
@fig-SEN12MS 에는 지리적 정보를 담은 metadata가 있어서, 명확하게 지역을 구분지을수 있었고, 이를 통해서 2-way 2-shot classification task, 즉 2개의 이미지만 보고 2개의 class를 구분하는 task를 수행하는 예시도 같이 공유했다. 반면 @fig-DeepGlobe 에서 소개된 DeepGlobe에는 metadaata가 없어서, Clustering을 사용해서 임의의 지역을 구분해서 주어진 image에 대한 segmentation이 잘 수행되는지를 확인했다.
...
- **Random Initialization** - 새로운 지역 데이터 $\mathcal{D}^{tr}_j$ 로 처음부터 학습하는 방식
...
을 두었고, 이에 대한 결과는 @fig-results 와 같다.

![result of meta learning](./images/37p_result_meta_learning.png){#fig-results}

확실히 지리적 정보가 데이터셋에서 제공되는 SEN12MS에서는 MAML방식이 다른 방식들에 비해서 좋은 성능을 보여주며, 새로운 환경에 빠르게 적응하도록 최적화되었음 확인할 수 있었다. 물론 전혀 학습되지 않은 데이터가 들어온 케이스인 zero-shot 경우에는 MAML보다 Pre-train 모델이 성능이 좋았지만, 이는 지역의 평균적인 특징이 pre-train을 통해서 학습된 케이스였고, 데이터가 조금씩 들어오는 1-shot 케이스부터는 MAML의 정확도가 급격히 올라갔다. Segmentation 정확도를 확인하는 DeepGlobe 데이터셋에 대해서도 clustering을 통한 임의의 정보가 부여될 경우, MAML 의 성능이 다른 방법론보다 좋게 나온 것을 확인할 수 있다.

## Summary

이번 포스트에서 다룬 강의에서는 큰 주제였던 **Optimization-based Meta Learning**에 대해서 설명했단,. 이전 장에서 설명했던 **Black-Box Adaptation**이 Black-Box 신경망이 학습 데이터로부터 알고리즘을 학습하는 방식이라면, **Optimization-based Adaptation**은 inner-loop와 outer-loop를 통해서 다양한 task에 빠르게 adaptation할 수 있는 초기 parameter를 찾는 일종의 **Bi-level optimization** 구조로 되어 있다. 특히 모델의 구조와 상관없이 gradient descent를 통해서 최적화가 이뤄지기 때문에 **Model-Agnostic**이라는 표현이 같이 소개된다. 전자의 경우는 표현력이 좋지만, 테스트 데이터의 분포가 학습 데이터의 분포와 다른 Out-of-Distribution 케이스에서는 성능이 급격이 떨어지는 단점이 있으며, 후자의 경우에는 OoD 데이터가 들어와도 내부의 Fine-tuning 과정을 통해서 Extrapolation이 잘 이뤄진다는 특징을 가진다.
...
MAML은 Few-shot Learning과 같은 문제에서 좋은 성능을 보여주기는 하지만, 학습의 불안정성과 computation cost가 크다는 한계가 내재되어 있어서, 강의를 통해서 해당 부분을 극복하기 위해 시도된 **AutoMeta**나 **Reptile**, **R2-D2**같은 다양한 연구에 대해서 소개하기도 했다.
