...
\begin{algorithmic}
\State save target netowkr parameters: ${\color{red}\phi'} \leftarrow \phi$
\Forall{N}
\State collect data $\{(s_i, a_i, r_i, s_i')\}$ using some policy, add it to $\mathcal{R}$
  \Forall{K}
    \State sample a batch $(s_i, a_i, r_i, s_i')$ from $\mathcal{R}$
    \State $\phi \leftarrow - \alpha \sum_i \frac{d Q_{\phi}}{d \phi} (s_i, a_i) (Q_{\phi}(s_i, a_i) - [r(s_i, a_i) + \gamma \max_{a'}{\color{red}Q_{\phi'}(s_i', a_i')}])$
  \EndFor
\EndFor
\end{algorithmic}
\end{algorithm}
