...
$$
L(\theta) = \mathbb{E}_{\textcolor{red}{(s, a, s', a')} \sim \mathcal{D}} [(r(s, a) + \textcolor{red}{\gamma Q_{\hat{\theta}}(s', a')} - Q_{\theta}(s, a))^2]
$$ {#eq-SARSA-Error-Loss}
...
$$ 
Q_{\theta^{*}}(s, a) \approx r(s, a) + \gamma \mathcal{E}_{s' \sim p(\cdot|s, a) \\ a' \sim \pi_{\beta}(\cdot|s)}[Q_{\hat{\theta}}(s', a')]
$$ {#eq-ideal-Q-function}
